/* window_ios.m  -  Window library  -  Public Domain  -  2014 Mattias Jansson
 *
...
window_add_displaylink(window_t* window, window_draw_fn drawfn) {
	...
	id display_link =
	    [NSClassFromString(@"CADisplayLink") displayLinkWithTarget:wrapper
	                                                      selector:@selector(callDraw:)];
...
- (id)initWithFrame:(CGRect)frame {
	if ((self = [super initWithFrame:frame])) {
		...
		log_debugf(HASH_WINDOW,
		           "WindowView initWithFrame, setting up layer (class %s) %dx%d (%.1f)",
		           class_getName([[self layer] class]), screen_width, screen_height,
...
- (id)initWithCoder:(NSCoder*)coder {
	if ((self = [super initWithCoder:coder])) {
		...
		log_debugf(HASH_WINDOW,
		           "WindowView initWithCoder, setting up layer (class %s) %dx%d (%.1f)",
		           class_getName([[self layer] class]), (int)screen_width, (int)screen_height,
		           self.contentScaleFactor);
		CAEAGLLayer* layer = (CAEAGLLayer*)self.layer;
		layer.opaque = TRUE;
		if (self.contentScaleFactor > 1)
